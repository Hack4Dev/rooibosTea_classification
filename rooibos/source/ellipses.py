...
def draw_confidence_ellipse (xFer, yFer, xNF, yNF, xaxis, yaxis, sol, xl, yl):
    ...
    ax_nstd.scatter(np.array(xFer).flatten(), np.array(yFer).flatten(), s=6, color = 'blue')
    confidence_ellipse(np.array(xFer).flatten(), np.array(yFer).flatten(), ax_nstd, n_std=2,label='FR', edgecolor='blue', linestyle='--')


    ax_nstd.scatter(np.array(xNF).flatten(),np.array(yNF).flatten(), s=6, color = 'orange')

    confidence_ellipse(np.array(xNF).flatten(), np.array(yNF).flatten(), ax_nstd, n_std=2,label='NF', edgecolor='orange', linestyle='--')
